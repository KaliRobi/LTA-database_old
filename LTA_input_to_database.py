...
class Record:
    def __init__(self, volume, id, name_with_aliases, sex, height_cm, build, dentition, special_peculiarities, date_of_birth, place_of_birth, place_of_residence, residence, religion, childhood_status, marital_status, number_of_children, occupation,
        ...
        self.notes = notes
    #setter properties are needed to be able to reset the values during correction
...
def modify_hash(dicts):
    ...
    wrong_val = record_hash[wrong_key]
    record_hash[wrong_key] = input(f"{wrong_key} is {wrong_val} now, it should be : \n")
    print(dicts)


def correction(val):
    while val != 'yes':
        modify_hash(record_hash)
...
vlarr = [
    ...
    notes,
    arrest_site]
record_dict = dict.fromkeys(record_keys)

record_hash = {k: v for k, v in zip(record_dict, vlarr)}


visual_check = input("all the data s correct? \n")
correction(visual_check)

#the new data needs to be updated to the new_record
